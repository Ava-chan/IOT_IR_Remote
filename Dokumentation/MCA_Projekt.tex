...
Wird er ohne PC betrieben kann er über ein einfaches 5V USB Netzteil auch als Zuleitung verwendet werden. Die TX und RX-Leitungen liegen dann auf 0V.
...
Aufgrund dessen, dass Senden und Empfangen nicht gleichzeitig möglich sind, bedarf es keiner Abschirmung zwischen Sende und Empfangsdiode. Es wird über die Programmierung sichergestellt das der Sender während des erwarteten Erfassen von Signalen nicht leuchtet (Ausgang liegt auf High).
...
\node[text width=4cm] at (6.00,0.45) {\footnotesize Anzahl};
...
Wird auf der Startseite ein Klick auf \qq{Record New} ausgeführt, dann können Infrarotcodes aufgenommen, bezeichnet und abgespeichert werden.
...
Die restlichen 0.3 $\mu$s werden ca. von den Ansteuerungsfunktionen für das Wechseln zwischen 1 und 0 des Ausgangs sowie der Schleife verwendet.
...
Die ganze Realisierung ist im direkten Vergleich zu einer Logitech MyHarmony Universalfernbedingung sehr kostengünstig.
...
Mit mDNS sowie dessen Unterstützung kann einfach über \url{iot_remote.local} zugegriffen werden, sodass die IP-Adresse nicht bekannt sein muss.
Im AcessPoint-Modus wird immer über die IP-Adresse \url{192.168.4.1} zugegriffen.

Es besteht natürlich die Einschränkung, dass nur Geräte, welche mit einer Trägerfrequenz von 38kHz arbeiten, unterstützt werden.
